...
void task_trajectory_generator_proc(void *arg)
{
	...
	while (1) {
		...
		printf("Task A Time since last turn: %ld.%06ld ms\n",
		  ...
		  //axis1_setpoint.Position = 0;
          	  //axis1_setpoint.Velocity = 0;
		  axis1_setpoint.Position+=1*axis1_setpoint.Velocity;
		  ...
		  if((new_cmd.Position-axis1_setpoint.Position)>(new_cmd.Velocity*new_cmd.Velocity/(2*new_cmd.Deceleration))){
			if(axis1_setpoint.Velocity<new_cmd.Velocity){
				if((axis1_setpoint.Velocity+new_cmd.Acceleration*1)<new_cmd.Velocity){
					axis1_setpoint.Velocity+=new_cmd.Acceleration*1;
				}else{
					axis1_setpoint.Velocity=new_cmd.Velocity;
				}
			}
		  }else{
			if(axis1_setpoint.Velocity-new_cmd.Deceleration*1>0){
				axis1_setpoint.Velocity-=new_cmd.Deceleration*1;
			}	
		  }
